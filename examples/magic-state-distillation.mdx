...
description: Simulate magic state distillation protocols with tsim, including 85-qubit circuits using the [[17,1,5]] color code for fault-tolerant quantum computation
---
...
### Analyzing Rotation Angle Dependence
...
2. **Distill 5 encoded magic states into a single logical qubit.** We use the transversal encoding of the distillation circuit shown above. This includes depolarization noise with strength `noise`.
3. **Perform post-selection** We will perform two stages of post-selection: (a) post-select on perfect stabilizers, and (b) post-select on the [1,0,1,1] distillation syndrome of logical bits.
4. **Quantum state tomography** We will perform quantum state tomography on the output logical qubit to estimate its magic state fidelity.
